#define hanalysis_fMC_PID_cxx
// The class definition in hanalysis_fMC_PID.h has been generated automatically
...
//
// root> T->Process("hanalysis_fMC_PID.C")
// root> T->Process("hanalysis_fMC_PID.C","some options")
// root> T->Process("hanalysis_fMC_PID.C+")
//


#include "hanalysis_fMC_PID.h"
#include <TH2.h>
#include <TStyle.h>
#include "TH2D.h"
#include "TH1D.h"
...
void hanalysis_fMC_PID::Begin(TTree * /*tree*/)
{
...
void hanalysis_fMC_PID::SlaveBegin(TTree * /*tree*/)
{
   ...
   TString option = GetOption();
   fanalysis_PID= new TFile("/uboone/app/users/rcastill/ubooneprefilterv06_26_01_13XSEC/work/DataMC/MC/analysis_PID_BNBMC.root","RECREATE");
   outfileMC_PID.open("/uboone/app/users/rcastill/ubooneprefilterv06_26_01_13XSEC/work/DataMC/MC/info_PID_MC.txt");
...
///proton properties
  ...
  //////
   ...
   selmuon_mom_res_PID = new TH2D("selmuon_mom_res_PID", "selmuon_mom_res_PID",10, 0, 2, 10,0,2);
   selmuon_costheta_res_PID = new TH2D("selmuon_costheta_res_PID", "selmuon_costheta_res_PID", 10, -1, 1,10,-1,1);
   selmuon_phi_res_PID = new TH2D("selmuon_phi_res_PID", "selmuon_phi_res_PID",10, -3.1415, 3.1415, 10,-3.1415, 3.1415);

   selproton1_mom_res_PID = new TH2D("selproton1_mom_res_PID", "selproton1_mom_res_PID",10, 0, 2, 10,0,2);
   selproton1_costheta_res_PID = new TH2D("selproton1_costheta_res_PID", "selproton1_costheta_res_PID",10, -1, 1, 10,-1,1);
   selproton1_phi_res_PID = new TH2D("selproton1_phi_res_PID", "selproton1_phi_res_PID",10, -3.1415, 3.1415, 10,-3.1415, 3.1415);

   selproton2_mom_res_PID = new TH2D("selproton2_mom_res_PID", "selproton2_mom_res_PID",10, 0, 1.5, 10,0,1.5);
   selproton2_costheta_res_PID = new TH2D("selproton2_costheta_res_PID", "selproton2_costheta_res_PID",10, -1, 1, 10,-1,1);
   selproton2_phi_res_PID = new TH2D("selproton2_phi_res_PID", "selproton2_phi_res_PID",10, -3.1415, 3.1415, 10,-3.1415, 3.1415);
...
Bool_t hanalysis_fMC_PID::Process(Long64_t entry)
{
   ...
   if(track_pfp_Id.GetSize()==3){/// PID requirement
     ...
     if(muind !=-1 &&p1ind !=-1 && p2ind !=-1 && tempflag){/// requirements
       ...
       */
       ...
       selmuon_mom_res_PID->Fill(pfp_mom[muind],*track_pfp_mom_mucand);
       selmuon_costheta_res_PID->Fill( pfp_costheta[muind],track_pfp_costheta[muind]);
       selmuon_phi_res_PID->Fill(pfp_phi[muind],track_pfp_phi[muind]);

       selproton1_mom_res_PID->Fill(pfp_mom[p1ind],track_pfp_Mom_p[p1ind]);
       selproton1_costheta_res_PID->Fill(pfp_costheta[p1ind],track_pfp_costheta[p1ind]);
       selproton1_phi_res_PID->Fill(pfp_phi[p1ind],track_pfp_phi[p1ind]);

       selproton2_mom_res_PID->Fill(pfp_mom[p2ind],track_pfp_Mom_p[p2ind]);
       selproton2_costheta_res_PID->Fill(pfp_costheta[p2ind],track_pfp_costheta[p2ind]);
       selproton2_phi_res_PID->Fill(pfp_phi[p2ind],track_pfp_phi[p2ind]);
...
void hanalysis_fMC_PID::SlaveTerminate()
{
...
void hanalysis_fMC_PID::Terminate()
{
  ...
  //TProfile* muon = (TProfile*) dEdxrr->Get("muon");
  //TProfile* proton = (TProfile*) dEdxrr->Get("proton");
    ...
    //  TGraph* muon = (TGraph*) dEdxrr->Get("muon");
    //TGraph* proton = (TGraph*) dEdxrr->Get("proton");
  ...
  proton->SetLineColor(kBlack);
  proton->Draw("C same");
  

  TLegend *l1 = new TLegend(0.5, 0.7, 0.9, 0.9);
  //l1 -> SetNColumns(2);
  l1 -> AddEntry(muon, "muon expectation", "lep");
  l1 -> AddEntry(proton, "proton expectation", "lep");
  l1 -> AddEntry(hprof_all_MC, "<dE/dx> in residual range", "lep");
  l1 -> Draw();  
  */
  c1->Update();
  c1-> Print("/uboone/app/users/rcastill/ubooneprefilterv06_26_01_13XSEC/work/DataMC/Plots/PID_300_tune3/selall_dEdx_vs_resrange_all_PID.pdf");
  ...
  */
  ...
  //l2 -> SetNColumns(2);
  l2 -> AddEntry(hprof_selprotons_MC, "<dE/dx> per residual range in selected protons", "lep");
  l2 -> AddEntry(muon, "muon expectation", "lep");
  l2 -> AddEntry(proton, "proton expectation", "lep");
  l2 -> Draw();  
  */
  c2->Update();
  c2-> Print("/pnfs/uboone/persistent/users/rcastill/CC0Pion2P/Plots/BNBCosmicMC/cutsPID/hprof_protons.pdf");
